...
- [See also](#see-also)
- [Notes](#notes)
- [Updating the storefront GraphQL schema](#updating-the-storefront-graphql-schema)
...
## Updating the storefront GraphQL schema

`packages/core/schema/storefront.graphql` is a vendored copy of the [storefront](https://github.com/bigcommerce/storefront) GraphQL SDL. The TypeScript types and documents in `packages/core/src/generated-codegen` are generated from it (see `packages/core/codegen.ts`) and are not committed — they are regenerated automatically before every build, lint and test run.

To refresh the schema:

1. Copy `src/main/resources/graphql/autogenerated/schema.graphql` from the storefront repository into `packages/core/schema/storefront.graphql`.

2. Fix the invalid `@cacheControl` scope values — the upstream SDL applies `@cacheControl(scope: Public)` while the enum value is `PUBLIC`, which strict validation rejects:

    ```sh
    sed -i '' 's/scope: Public/scope: PUBLIC/g' packages/core/schema/storefront.graphql
    ```
