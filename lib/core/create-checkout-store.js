...
function createCheckoutSelectors(state, cacheFactory, options) {
    ...
    var checkout = new checkout_1.CheckoutSelector(billingAddress, cart, config, countries, customer, instruments, order, paymentMethods, quote, shippingAddress, shippingCountries, shippingOptions, cacheFactory);
    var errors = new checkout_1.CheckoutErrorSelector(billingAddress, cart, countries, coupon, customer, giftCertificate, instruments, order, paymentMethods, quote, shippingAddress, shippingCountries, shippingOptions);
    var statuses = new checkout_1.CheckoutStatusSelector(billingAddress, cart, countries, coupon, customer, giftCertificate, instruments, order, paymentMethods, quote, shippingAddress, shippingCountries, shippingOptions);
    return {
...
function createCheckoutStore(initialState, options) {
    ...
    return data_store_1.createDataStore(createCheckoutReducers(), initialState, function (state) { return createCheckoutSelectors(state, cacheFactory, options); }, options);
}
