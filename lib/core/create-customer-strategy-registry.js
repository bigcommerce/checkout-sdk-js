...
function createCustomerStrategyRegistry(store, client) {
    ...
    registry.register('amazon', function () {
        return new strategies_1.AmazonPayCustomerStrategy(store, signInCustomerService, new remote_checkout_1.RemoteCheckoutRequestSender(request_sender_1.createRequestSender()), new amazon_pay_1.AmazonPayScriptLoader(script_loader_1.createScriptLoader()));
    });
