...
describe('CheckoutSelector', function () {
    ...
    beforeEach(function () {
        ...
        orderSelector = new order_1.OrderSelector(state.order);
        selector = new checkout_selector_1.default(new billing_1.BillingAddressSelector(state.quote), new cart_1.CartSelector(state.cart), new config_1.ConfigSelector(state.config), new geography_1.CountrySelector(state.countries), new customer_1.CustomerSelector(state.customer), orderSelector, new payment_1.PaymentMethodSelector(state.paymentMethods), new quote_1.QuoteSelector(state.quote), new shipping_1.ShippingAddressSelector(state.quote), new shipping_1.ShippingCountrySelector(state.shippingCountries), new shipping_1.ShippingOptionSelector(state.shippingOptions));
    });
    ...
    it('returns flag indicating if payment is submitted', function () {
        jest.spyOn(orderSelector, 'isPaymentDataSubmitted');
        expect(selector.isPaymentDataSubmitted('braintree')).toEqual(true);
        expect(orderSelector.isPaymentDataSubmitted).toHaveBeenCalledWith(payment_methods_mock_1.getBraintree());
    });
});
