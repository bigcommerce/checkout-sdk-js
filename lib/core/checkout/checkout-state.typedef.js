"use strict";
/**
 * @typedef {Object} CheckoutState
 * @property {CartState} cart
 * @property {ConfigState} config
 * @property {CountriesState} countries
 * @property {CouponState} coupons
 * @property {CustomerState} customer
 * @property {GiftCertificateState} giftCertificates
 * @property {InstrumentState} instruments
 * @property {OrderState} order
 * @property {PaymentState} payment
 * @property {PaymentMethodsState} paymentMethods
 * @property {QuoteState} quote
 * @property {ShippingCountriesState} shippingCountries
 * @property {ShippingOptionsState} shippingOptions
 */
//# sourceMappingURL=checkout-state.typedef.js.map