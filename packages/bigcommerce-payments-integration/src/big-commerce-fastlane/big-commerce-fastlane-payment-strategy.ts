...
export default class BigCommerceFastlanePaymentStrategy implements PaymentStrategy {
     ...
     * */
    async initialize(
        options: PaymentInitializeOptions & WithBigCommerceFastlanePaymentInitializeOptions,
    ): Promise<void> {
    ...
    private async prepareVaultedInstrumentPaymentPayload(
        ...
        const { orderId } = await this.bigCommerceRequestSender.createOrder(methodId, {
            cartId,
        });
