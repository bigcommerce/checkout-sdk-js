...
import BigCommerceCreditCardsPaymentInitializeOptions, {
    WithBigCommerceCreditCardsPaymentInitializeOptions,
} from './big-commerce-credit-cards-payment-initialize-options';
import BigCommerceCreditCardsPaymentStrategy from './big-commerce-credit-cards-payment-strategy';

describe('BigCommerceCreditCardsPaymentStrategy', () => {
    let billingAddress: BillingAddress;
    let cart: Cart;
    let strategy: BigCommerceCreditCardsPaymentStrategy;
    ...
    let bigCommerceCardNameFieldElement: HTMLDivElement;

    const methodId = 'bigcommerce_payments_creditcard';

    const bigCommerceCardNameFieldContainerId = 'card-name';
    ...
    const creditCardFormFields = {
        ...
        [HostedFieldType.CardName]: { containerId: bigCommerceCardNameFieldContainerId },
    };
    ...
    const bigCommerceCreditCardsOptions: BigCommerceCreditCardsPaymentInitializeOptions = {
        form: {
    ...
    const initializationOptions: PaymentInitializeOptions &
        WithBigCommerceCreditCardsPaymentInitializeOptions = {
        methodId,
        bigcommerce_payments_creditcard: bigCommerceCreditCardsOptions,
    };

    const defaultExecutePayload = {
        payment: {
            methodId: 'bigcommerce_payments_creditcard',
            paymentData: {},
        },
    };
    ...
    beforeEach(() => {
        ...
        strategy = new BigCommerceCreditCardsPaymentStrategy(
            paymentIntegrationService,
        ...
        bigCommerceCardNameFieldElement = document.createElement('div');
        bigCommerceCardNameFieldElement.id = bigCommerceCardNameFieldContainerId;
        document.body.appendChild(bigCommerceCardNameFieldElement);
        ...
        jest.spyOn(bigCommerceIntegrationService, 'createOrderCardFields').mockResolvedValue({
            orderId: 'orderId',
            setupToken: 'setupToken',
        });

        jest.spyOn(bigcommerceSdk, 'CardFields').mockImplementation(
            (options: BigCommerceCardFieldsConfig) => {
    ...
    afterEach(() => {
        ...
        if (document.getElementById(bigCommerceCardNameFieldContainerId)) {
            document.body.removeChild(bigCommerceCardNameFieldElement);
        }
    });

    it('creates an interface of the BigCommerce Credit Cards payment strategy', () => {
    ...
    describe('#initialize', () => {
        ...
        it('throws an error if bigcommerce_payments_creditcard.form option is not provided', async () => {
        ...
        it('loads bigcommerce_payments_creditcard payment method', async () => {
        ...
        it('loads bigcommerce fastlane sdk if bigcommerce fastlane analytic is enabled', async () => {
    ...
    describe('#renderFields', () => {
        it('throws an error if card field is not eligible', async () => {
            jest.spyOn(bigcommerceSdk, 'CardFields').mockResolvedValue({
        ...
        it('calls a callback from initialization options if there was an issue with form rendering process', async () => {
            ...
            expect(
                initializationOptions.bigcommerce_payments_creditcard
        ...
        it('renders card fields with valid options', async () => {
            const initializationOptionsWithStyles = {
                methodId,
                bigcommerce_payments_creditcard: {
                    form: {
                        fields: bigCommerceCreditCardsOptions.form.fields,
                        styles: {
                            default: {
                                fontFamily: 'bigFont',
                                fontSize: '14px',
            ...
            expect(bigcommerceSdk.CardFields).toHaveBeenCalledWith(cardFieldsOptionsMock);
        });
    });
