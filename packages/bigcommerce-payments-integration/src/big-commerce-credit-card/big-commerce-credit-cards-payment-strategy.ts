...
import {
    BigCommerceCardFields,
    BigCommerceCardFieldsConfig,
    BigCommerceCardFieldsOnApproveData,
    BigCommerceCardFieldsState,
    BigCommerceFields,
    BigCommerceHostedFieldsRenderOptions,
    LiabilityShiftEnum,
...
export default class BigCommerceCreditCardsPaymentStrategy implements PaymentStrategy {
    ...
    private cardFields?: BigCommerceCardFields;
    private cvvField?: BigCommerceFields;
    private expiryField?: BigCommerceFields;
    private numberField?: BigCommerceFields;
    private nameField?: BigCommerceFields;
    ...
    async initialize(
        options: PaymentInitializeOptions & WithBigCommerceCreditCardsPaymentInitializeOptions,
    ): Promise<void> {
        const { methodId, bigcommerce_payments_creditcard, bigcommerce } = options;
        const bigcommerceCommerceInitializationOptions =
            bigcommerce_payments_creditcard || bigcommerce;

        const { form, onCreditCardFieldsRenderingError } =
            bigcommerceCommerceInitializationOptions || {};
        ...
        if (!form) {
            throw new InvalidArgumentError(
                'Unable to proceed because "options.bigcommerce_payments_creditcard.form" argument is not provided.',
            );
        }
        ...
        await this.bigCommerceIntegrationService.loadBigCommerceSdk(
            methodId,
            undefined,
            true,
            true,
    ...
    async execute(payload: OrderRequestBody, options?: PaymentRequestOptions): Promise<void> {
        ...
        if (this.isCreditCardForm || this.isCreditCardVaultedForm) {
            ...
            const { orderId } = await this.bigCommerceIntegrationService.createOrderCardFields(
    ...
    private async initializeFields(
        ...
        onCreditCardFieldsRenderingError?: BigCommerceCreditCardsPaymentInitializeOptions['onCreditCardFieldsRenderingError'],
    ): Promise<void> {
        ...
        const cardFieldsConfig: BigCommerceCardFieldsConfig = {
    ...
    private createVaultSetupTokenCallback() {
        return {
            createVaultSetupToken: async () => {
                const { setupToken } =
                    (await this.bigCommerceIntegrationService.createOrderCardFields(
                        'paypalcommercecreditcardscheckout',
                        {
                            ...this.getInstrumentParams(),
                            setupToken: true,
     ...
     *
     */
    private handleApprove({ orderID, vaultSetupToken }: BigCommerceCardFieldsOnApproveData) {
        if (orderID) {
            this.returnedOrderId = orderID;
        }
    ...
    private getValidityData({ fields }: BigCommerceCardFieldsState): HostedFieldValidateEventData {
        const updatedFields = { ...fields };

        delete updatedFields.cardNameField;

        const fieldsKeys = Object.keys(updatedFields) as Array<
            keyof BigCommerceCardFieldsState['fields']
        >;
     ...
     *
     */
    private getCardFieldsOrThrow(): BigCommerceCardFields {
        if (!this.cardFields) {
            throw new NotInitializedError(NotInitializedErrorType.PaymentNotInitialized);
        }
    ...
    ): BigCommerceHostedFieldsRenderOptions['styles'] {
        const commonStyles = {
    ...
    private onChangeHandler(
        formOptions: HostedFormOptions,
        event: BigCommerceCardFieldsState,
    ): void {
    ...
    private onBlurHandler(formOptions: HostedFormOptions, event: BigCommerceCardFieldsState): void {
