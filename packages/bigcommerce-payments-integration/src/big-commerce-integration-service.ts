...
import {
    ...
    BigCommerceCreateOrderCardFieldsResponse,
    BigCommerceCreateOrderRequestBody,
    BigCommerceInitializationData,
    BigCommerceOrderDetails,
...
export default class BigCommerceIntegrationService {
    private bigCommerceSdk?: BigCommerceSDK;

    constructor(
        private formPoster: FormPoster,
        private paymentIntegrationService: PaymentIntegrationService,
        private bigCommerceRequestSender: BigCommerceRequestSender,
        private bigCommerceScriptLoader: BigCommerceScriptLoader,
    ) {}

    /**
     *
     * BigCommerceSDK methods
     *
     */
    async loadBigCommerceSdk(
        methodId: string,
        providedCurrencyCode?: string,
        initializesOnCheckoutPage?: boolean,
        forceLoad?: boolean,
    ): Promise<BigCommerceSDK | undefined> {
        ...
        this.bigCommerceSdk = await this.bigCommerceScriptLoader.getBigCommerceSDK(
            paymentMethod,
            currencyCode,
        ...
        return this.bigCommerceSdk;
    }

    getBigCommerceSdkOrThrow(): BigCommerceSDK {
        if (!this.bigCommerceSdk) {
    ...
    async createOrder(
        ...
        const { orderId } = await this.bigCommerceRequestSender.createOrder(providerId, {
            cartId,
            ...requestBody,
        });
    ...
    async createOrderCardFields(
        providerId: string,
        requestBody?: Partial<BigCommerceCreateOrderRequestBody>,
    ): Promise<BigCommerceCreateOrderCardFieldsResponse> {
        const cartId = this.paymentIntegrationService.getState().getCartOrThrow().id;

        const { orderId, setupToken } = await this.bigCommerceRequestSender.createOrder(
            providerId,
            {
                cartId,
                ...requestBody,
    ...
    async updateOrder(): Promise<void> {
        ...
        try {
            await this.bigCommerceRequestSender.updateOrder({
    ...
    ): Promise<BigCommerceOrderStatus> {
        try {
            const { status } = await this.bigCommerceRequestSender.getOrderStatus(
                methodId,
                options,
            );
    ...
    getBillingAddressFromOrderDetails({
        payer,
    }: BigCommerceOrderDetails): BillingAddressRequestBody {
    ...
    getShippingAddressFromOrderDetails(
        orderDetails: BigCommerceOrderDetails,
    ): BillingAddressRequestBody {
     ...
     *
     */
    getValidButtonStyle(style?: BigCommerceButtonStyleOptions): BigCommerceButtonStyleOptions {
