...
import {
    BigCommerceRatePay,
    WithBigCommerceRatePayPaymentInitializeOptions,
} from './big-commerce-ratepay-initialize-options';
...
export default class BigCommerceRatepayPaymentStrategy implements PaymentStrategy {
    ...
    async initialize(
        options: PaymentInitializeOptions & WithBigCommerceRatePayPaymentInitializeOptions,
    ): Promise<void> {
        ...
        const paymentMethod = state.getPaymentMethodOrThrow<BigCommerceInitializationData>(
            methodId,
            gatewayId,
        );
    ...
    private async initializePollingMechanism(
        ...
        try {
            ...
            const orderStatus = await this.bigCommerceIntegrationService.getOrderStatus(
            ...
            const isOrderApproved = orderStatus === BigCommerceOrderStatus.PollingStop;
            const isPollingError = orderStatus === BigCommerceOrderStatus.PollingError;
