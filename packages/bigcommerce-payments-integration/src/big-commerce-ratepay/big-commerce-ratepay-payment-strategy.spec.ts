...
import {
    ...
    getBigCommerceRatePayPaymentMethod,
...
describe('PayPalCommerceAlternativeMethodRatePayPaymentStrategy', () => {
    ...
    let strategy: BigCommerceRatepayPaymentStrategy;
    let loadingIndicator: LoadingIndicator;

    const bigCommerceRatePayMethodsOptions: BigCommerceRatePay = {
    ...
    beforeEach(() => {
        ...
        strategy = new BigCommerceRatepayPaymentStrategy(
            paymentIntegrationService,
        ...
        jest.spyOn(bigCommerceIntegrationService, 'getOrderStatus').mockResolvedValue(
            BigCommerceOrderStatus.PollingStop,
        );
        ...
        bigCommerceSdk.Legal.FUNDING = {
            PAY_UPON_INVOICE: 'PAY_UPON_INVOICE',
        };

        paymentMethod = getBigCommerceRatePayPaymentMethod();
        ...
        jest.spyOn(bigCommerceIntegrationService, 'createOrder').mockResolvedValue('1eddfd');
    ...
    describe('#execute()', () => {
        ...
        it('initialize polling mechanism', async () => {
            ...
            expect(bigCommerceIntegrationService.getOrderStatus).toHaveBeenCalled();
        });

        it('stop polling mechanism if corresponding status received', async () => {
            jest.spyOn(bigCommerceIntegrationService, 'getOrderStatus').mockResolvedValue(
                BigCommerceOrderStatus.PollingError,
            );
