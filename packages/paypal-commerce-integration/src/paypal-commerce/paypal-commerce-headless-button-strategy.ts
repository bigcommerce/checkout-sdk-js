...
import { WithPayPalCommerceHeadlessButtonInitializeOptions } from './paypal-commerce-headless-button-initialize-options';

export default class PaypalCommerceHeadlessButtonStrategy implements CheckoutButtonStrategy {
    constructor(
    ...
    async initialize(
        options: CheckoutButtonInitializeOptions &
            WithPayPalCommerceHeadlessButtonInitializeOptions,
    ): Promise<void> {
        ...
        const state = await this.paymentIntegrationService.loadCardEntity(paypalcommerce.cartId);
