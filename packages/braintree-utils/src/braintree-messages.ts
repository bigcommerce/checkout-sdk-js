...
import { MessagesStyleOptions } from './paypal';
...
export default class BraintreeMessages {
    ...
    render(methodId: string, containerId: string, placement: 'cart' | 'payment'): void {
        const isValidContainer = containerId && Boolean(document.getElementById(containerId));

        if (this.braintreeHostWindow.paypal && isValidContainer) {
            ...
            if (paypalBNPLConfiguration) {
                const bannedId = placement === 'payment' ? 'checkout' : placement;
