...
export default async function createEnum({
    ...
    const filePaths = (await Promise.all(inputPaths.map(inputPath => promisify(glob)(inputPath))))
        .flatMap(members => members);

    const enumMembers = (await Promise.all(filePaths.map(filePath => getEnumMembers(filePath, inputMemberPattern))))
        .flatMap(members => members);
        ...
        .printList(
            ...
            ts.factory.createNodeArray([
                createEnumDeclaration(
                    outputMemberName,
                    deduplicateEnumMembers(enumMembers)
                ),
...
async function getEnumMembers(
    filePath: string,
    memberPattern: string
): Promise<Array<ts.EnumMember>> {
    ...
    const memberPaths = root.statements
        ...
        .filter(statement => {
            ...
            return statement.exportClause.elements.filter(ts.isExportSpecifier)
                .find(element => element.name.escapedText.toString().match(new RegExp(memberPattern)));
        })
        .map(statement => statement.moduleSpecifier?.getText(root)?.replace(/'|"/g, ''))
        .filter(exists);

    const memberRootPaths = await Promise.all(memberPaths.map(memberPath => toRootPath(memberPath, filePath)));
    ...
    return (await Promise.all(
        memberRootPaths.map(async rootPath => {
            ...
            return (await getSource(rootPath)).statements
                ...
                .filter(statement => statement.name.escapedText.toString().match(new RegExp(memberPattern)))
                .flatMap(statement => statement.members);
        })
    )).flatMap(enumMembers => enumMembers);
}

function deduplicateEnumMembers(enumMembers: Array<ts.EnumMember>): Array<ts.EnumMember> {
    const result = enumMembers.reduce(
        (map, enumMember) => ts.isIdentifier(enumMember.name) ?
            map.set(enumMember.name.text, enumMember) :
            map,
        new Map()
    );
...
function createEnumDeclaration(
    enumName: string,
    enumMembers: Array<ts.EnumMember>
): ts.EnumDeclaration {
