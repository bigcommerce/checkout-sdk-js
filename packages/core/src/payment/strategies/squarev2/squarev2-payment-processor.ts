...
export default class SquareV2PaymentProcessor {
    ...
    async initialize({ testMode, applicationId, locationId }: SquareV2PaymentProcessorOptions): Promise<void> {
    ...
    private _getPayments(): Payments {
        return guard(this._payments, () => new NotInitializedError(NotInitializedErrorType.PaymentNotInitialized));
    }

    async initializeCard({ containerId, style, onValidationChange }: SquareV2PaymentInitializeOptions): Promise<void> {
        const { postalCode } = this._store.getState().billingAddress.getBillingAddress() || {};
        ...
        } catch (_error) { /* Do nothing: we should not block shoppers from buying. */ }

        if (onValidationChange) {
            this._formValidationSubscription = this._subscribeToFormValidation(this._card, onValidationChange);
        }
    }
    ...
    private _subscribeToFormValidation(
        ...
        const invalidFields = new Set<string>(['cardNumber', 'expirationDate', 'cvv', 'postalCode']);
        ...
        return merge(...eventObservables)
            .pipe(
                map((event: SqEvent<CardInputEvent>): boolean => {
                    const { detail: { field, currentState: { isCompletelyValid } } } = event;
    ...
    private _getCard(): Card {
        return guard(this._card, () => new NotInitializedError(NotInitializedErrorType.PaymentNotInitialized));
    }
