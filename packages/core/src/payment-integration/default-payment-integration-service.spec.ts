...
describe("DefaultPaymentIntegrationService", () => {
    ...
    >;
    let storeProjectionFactory: Pick<
        PaymentIntegrationStoreProjectionFactory,
        "create"
    >;
    ...
    let orderActionCreator: Pick<
        OrderActionCreator,
        "submitOrder" | "finalizeOrder"
    >;
    let billingAddressActionCreator: Pick<
    ...
    beforeEach(() => {
        ...
        checkoutActionCreator = {
            loadCurrentCheckout: jest.fn(
                async () => () => createAction("LOAD_CHECKOUT")
            ),
            loadDefaultCheckout: jest.fn(
                async () => () => createAction("LOAD_CHECKOUT")
            ),
        };
        ...
        orderActionCreator = {
            submitOrder: jest.fn(
                async () => () => createAction("SUBMIT_ORDER")
            ),
            finalizeOrder: jest.fn(
                async () => () => createAction("FINALIZE_ORDER")
            ),
        };

        billingAddressActionCreator = {
            updateAddress: jest.fn(
                async () => () => createAction("UPDATE_BILLING_ADDRESS")
            ),
        };

        consignmentActionCreator = {
            updateAddress: jest.fn(
                async () => () => createAction("UPDATE_CONSIGNMENT_ADDRESS")
            ),
            selectShippingOption: jest.fn(
                async () => () => createAction("UPDATE_SHIPPING_OPTION")
            ),
        };

        paymentMethodActionCreator = {
            loadPaymentMethod: jest.fn(
                async () => () => createAction("LOAD_PAYMENT_METHOD")
            ),
        };
    ...
    describe("#loadPaymentMethod", () => {
        it("loads payment method", async () => {
            ...
            expect(
                paymentMethodActionCreator.loadPaymentMethod
    ...
    describe("#submitOrder", () => {
        it("submits order", async () => {
            ...
            expect(store.dispatch).toHaveBeenCalledWith(
                orderActionCreator.submitOrder()
            );
            expect(output).toEqual(paymentIntegrationSelectors);
        });
    });
    ...
    describe("#updateBillingAddress", () => {
        it("update billing address", async () => {
            const output = await subject.updateBillingAddress(
                getBillingAddress()
            );
    ...
    describe("#updateShippingAddress", () => {
        it("update shipping address", async () => {
            const output = await subject.updateShippingAddress(
                getShippingAddress()
            );
    ...
    describe("#selectShippingOption", () => {
        it("select shipping option", async () => {
            ...
            expect(
                consignmentActionCreator.selectShippingOption
