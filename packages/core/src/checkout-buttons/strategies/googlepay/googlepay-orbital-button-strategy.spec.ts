...
describe('GooglePayCheckoutButtonStrategy', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(paymentProcessor, 'initialize').mockResolvedValue(Promise.resolve());

        jest.spyOn(store.getState().paymentMethods, 'getPaymentMethod').mockResolvedValue(
            paymentMethod,
        );

        jest.spyOn(store.getState().cart, 'getCartOrThrow').mockReturnValue(cart);

        jest.spyOn(formPoster, 'postForm').mockResolvedValue(Promise.resolve());
