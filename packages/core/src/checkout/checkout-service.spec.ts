...
describe("CheckoutService", () => {
    ...
    let paymentStrategyRegistryV2: ResolveIdRegistry<
        PaymentStrategyV2,
        PaymentStrategyResolveId
    >;
    ...
    beforeEach(() => {
        ...
        instrumentRequestSender = new InstrumentRequestSender(
            paymentClient,
            requestSender
        );
        ...
        shippingCountryRequestSender = new ShippingCountryRequestSender(
            requestSender,
            { locale }
        );
        ...
        billingAddressRequestSender = new BillingAddressRequestSender(
            requestSender
        );
        subscriptionsRequestSender = new SubscriptionsRequestSender(
            requestSender
        );
        ...
        jest.spyOn(paymentStrategy, "execute").mockResolvedValue(
            store.getState()
        );

        jest.spyOn(paymentStrategy, "finalize").mockResolvedValue(
            store.getState()
        );

        jest.spyOn(paymentStrategy, "initialize").mockResolvedValue(
            store.getState()
        );

        jest.spyOn(paymentStrategy, "deinitialize").mockResolvedValue(
            store.getState()
        );

        paymentStrategyRegistry = new PaymentStrategyRegistry(store);
        const paymentIntegrationService =
            createPaymentIntegrationService(store);
        paymentStrategyRegistryV2 = createPaymentStrategyRegistryV2(
        ...
        jest.spyOn(paymentStrategyRegistry, "getByMethod").mockReturnValue(
            paymentStrategy
        );

        orderRequestSender = new OrderRequestSender(requestSender);

        jest.spyOn(orderRequestSender, "loadOrder").mockResolvedValue(
        ...
        jest.spyOn(
            consignmentRequestSender,
            "createConsignments"
        ).mockResolvedValue(getResponse(getCheckout()));
        jest.spyOn(
            consignmentRequestSender,
            "updateConsignment"
        ).mockResolvedValue(getResponse(getCheckout()));

        giftCertificateRequestSender = new GiftCertificateRequestSender(
            requestSender
        );
        ...
        jest.spyOn(configRequestSender, "loadConfig").mockResolvedValue(
            getResponse(getConfig())
        );

        customerRequestSender = new CustomerRequestSender(requestSender);

        jest.spyOn(customerRequestSender, "createAccount").mockResolvedValue(
            getResponse({})
        );
        ...
        paymentMethodRequestSender = new PaymentMethodRequestSender(
            requestSender
        );
        ...
        formFieldsActionCreator = new FormFieldsActionCreator(
    ...
    describe("#loadCheckout()", () => {
        ...
        it("calls loadCheckout with the provided id", () => {
            ...
            expect(checkoutActionCreator.loadCheckout).toHaveBeenCalledWith(
                id,
                undefined
            );
        });
        ...
        it("returns a state with a hydrated checkout", async () => {
            ...
            expect(state.data.getCheckout()).toEqual(
                store.getState().checkout.getCheckout()
            );
        });
    });
    ...
    describe("#sendSignInEmail()", () => {
        it("sends sign-in email", async () => {
            ...
            expect(
                signInEmailRequestSender.sendSignInEmail
    ...
    describe("#createCustomerAddress()", () => {
        it("creates customer address", async () => {
            ...
            expect(customerRequestSender.createAddress).toHaveBeenCalledWith(
                address,
                undefined
            );
    ...
    describe("#updateSubscriptions()", () => {
        it("updates subscriptions", async () => {
            ...
            expect(
                subscriptionsRequestSender.updateSubscriptions
            ).toHaveBeenCalledWith(
                {
                    email: "foo@bar.com",
                    acceptsAbandonedCartEmails: true,
    ...
    describe("#loadShippingAddressFields()", () => {
        ...
        it("loads extra countries data", async () => {
            ...
            expect(state.data.getShippingCountries()).toEqual(
                getCountriesResponseBody().data
            );
        });
    });
    ...
    describe("#loadBillingAddressFields()", () => {
        ...
        it("loads extra countries data", async () => {
            ...
            expect(state.data.getBillingCountries()).toEqual(
                getCountriesResponseBody().data
            );
        });
    });
    ...
    describe("#loadOrder()", () => {
        ...
        it("loads order data", async () => {
            ...
            expect(state.data.getOrder()).toEqual(
                store.getState().order.getOrder()
            );
        });
    ...
    describe("#submitOrder()", () => {
        ...
        beforeEach(async () => {
            ...
            paymentStrategyRegistry.get = jest.fn(
                () => noPaymentDataRequiredPaymentStrategy
            );
        ...
        it("finds payment strategy", async () => {
            ...
            expect(paymentStrategyRegistry.getByMethod).toHaveBeenCalledWith(
                getAuthorizenet()
            );
        });
    ...
    describe("#loadPaymentMethods()", () => {
        it("loads payment methods", async () => {
            ...
            expect(
                paymentMethodRequestSender.loadPaymentMethods
    ...
    describe("#initializePayment()", () => {
        it("dispatches action to initialize payment", async () => {
            ...
            expect(
                paymentStrategyActionCreator.initialize
            ).toHaveBeenCalledWith(options);
    ...
    describe("#deinitializePayment()", () => {
        it("dispatches action to deinitialize payment", async () => {
            ...
            expect(
                paymentStrategyActionCreator.deinitialize
            ).toHaveBeenCalledWith(options);
            expect(store.dispatch).toHaveBeenCalledWith(action, {
                queueId: "paymentStrategy",
            });
    ...
    describe("#initializeCustomer()", () => {
        it("dispatches action to initialize customer", async () => {
            ...
            expect(
                customerStrategyActionCreator.initialize
            ).toHaveBeenCalledWith(options);
            expect(store.dispatch).toHaveBeenCalledWith(action, {
    ...
    describe("#deinitializeCustomer()", () => {
        it("dispatches action to deinitialize customer", async () => {
            ...
            expect(
                customerStrategyActionCreator.deinitialize
            ).toHaveBeenCalledWith(options);
            expect(store.dispatch).toHaveBeenCalledWith(action, {
                queueId: "customerStrategy",
            });
    ...
    describe("#continueAsGuest()", () => {
        it("dispatches action to continue as guest", async () => {
            ...
            ).toHaveBeenCalledWith({ email: "foo@bar.com" }, undefined);
    ...
    describe("#signInCustomer()", () => {
        it("dispatches action to sign in customer", async () => {
            ...
            jest.spyOn(customerStrategyActionCreator, "signIn").mockReturnValue(
                action
            );
    ...
    describe("#signOutCustomer()", () => {
        it("dispatches action to sign out customer", async () => {
            ...
            expect(customerStrategyActionCreator.signOut).toHaveBeenCalledWith(
                options
            );
            expect(store.dispatch).toHaveBeenCalledWith(action, {
                queueId: "customerStrategy",
    ...
    describe("#executePaymentMethodCheckout()", () => {
        it("dispatches action to execute payment method checkout", async () => {
            ...
            expect(
                customerStrategyActionCreator.executePaymentMethodCheckout
            ).toHaveBeenCalledWith(options);
            expect(store.dispatch).toHaveBeenCalledWith(action, {
                queueId: "customerStrategy",
            });
    ...
    describe("#loadShippingOptions()", () => {
        it("loads shipping options", async () => {
            ...
            expect(state.data.getShippingOptions()).toEqual(
                getShippingOptions()
            );
        });
    });
    ...
    describe("#initializeShipping()", () => {
        it("dispatches action to initialize shipping", async () => {
            ...
            expect(
                shippingStrategyActionCreator.initialize
            ).toHaveBeenCalledWith(options);
            expect(store.dispatch).toHaveBeenCalledWith(action, {
    ...
    describe("#deinitializeShipping()", () => {
        it("dispatches action to deinitialize shipping", async () => {
            ...
            expect(
                shippingStrategyActionCreator.deinitialize
            ).toHaveBeenCalledWith(options);
            expect(store.dispatch).toHaveBeenCalledWith(action, {
                queueId: "shippingStrategy",
            });
    ...
    describe("#selectConsignmentShippingOption()", () => {
        it("dispatches action to update shipping option for a consignment", async () => {
            ...
            await checkoutService.selectConsignmentShippingOption(
                "foo",
                "bar",
                options
            );
            ...
            expect(
                consignmentActionCreator.updateShippingOption
            ).toHaveBeenCalledWith(
                {
                    id: "foo",
    ...
    describe("#updateConsignment()", () => {
        it("dispatches action to update address for a consignment", async () => {
            ...
            expect(
                consignmentActionCreator.updateConsignment
    ...
    describe("#assignItemsToAddress()", () => {
        it("dispatches action to update consignment", async () => {
            ...
            expect(
                consignmentActionCreator.assignItemsByAddress
            ).toHaveBeenCalledWith(payload, options);
    ...
    describe("#unassignItemsToAddress()", () => {
        it("dispatches action to update consignment", async () => {
            ...
            expect(
                consignmentActionCreator.unassignItemsByAddress
            ).toHaveBeenCalledWith(payload, options);
    ...
    describe("#createConsignments()", () => {
        it("dispatches action to create consignments", async () => {
            ...
            expect(
                consignmentActionCreator.createConsignments
            ).toHaveBeenCalledWith(consignments, options);
    ...
    describe("#updateShippingAddress()", () => {
        it("dispatches action to update shipping address", async () => {
            ...
            expect(
                shippingStrategyActionCreator.updateAddress
    ...
    describe("#selectShippingOption()", () => {
        it("dispatches action to select shipping option", async () => {
            ...
            await checkoutService.selectShippingOption(
                shippingOptionId,
                options
            );

            expect(
                shippingStrategyActionCreator.selectOption
            ).toHaveBeenCalledWith(shippingOptionId, options);
            expect(store.dispatch).toHaveBeenCalledWith(action, {
                queueId: "shippingStrategy",
            });
        });
    ...
    describe("#updateBillingAddress()", () => {
        it("updates the billing address", async () => {
            ...
            expect(
                billingAddressRequestSender.updateAddress
    ...
    describe("#applyStoreCredit()", () => {
        it("applies store credit when called with true", async () => {
            ...
            expect(
                storeCreditRequestSender.applyStoreCredit
        ...
        it("removes store credit when called with false", async () => {
            ...
            expect(
                storeCreditRequestSender.removeStoreCredit
            ).toHaveBeenCalledWith(getCheckout().id, options);
        });
    });
    ...
    describe("#applyGiftCertificate()", () => {
        it("applies a gift certificate", async () => {
            ...
            expect(
                giftCertificateRequestSender.applyGiftCertificate
    ...
    describe("#removeGiftCertificate()", () => {
        it("removes a gift certificate", async () => {
            ...
            expect(
                giftCertificateRequestSender.removeGiftCertificate
            ).toHaveBeenCalledWith(getCheckout().id, code, options);
        });
    });
    ...
    describe("#deleteInstrument()", () => {
        it("deletes an instrument", async () => {
            ...
            expect(
                instrumentActionCreator.deleteInstrument
    ...
    describe("#initializeSpamProtection()", () => {
        ...
        beforeEach(() => {
            ...
            const action = of(
                createAction(SpamProtectionActionType.InitializeSucceeded)
            );
        ...
        it("initializes spam protection", async () => {
            ...
            expect(spamProtectionActionCreator.initialize).toHaveBeenCalledWith(
                options
            );
        });
    });
    ...
    describe("#executeSpamCheck()", () => {
        beforeEach(() => {
            ...
            jest.spyOn(
                ...
                from([
                    createAction(
                        SpamProtectionActionType.VerifyCheckoutRequested
                    ),
                    createAction(SpamProtectionActionType.ExecuteRequested),
                    createAction(SpamProtectionActionType.ExecuteSucceeded),
                    createAction(
