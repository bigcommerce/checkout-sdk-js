...
import {
    CustomerInitializeOptions, InvalidArgumentError, MissingDataError,
...
describe('BraintreeAcceleratedCheckoutCustomerStrategy', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(paymentIntegrationService, 'updateProviderCustomerData').mockImplementation(jest.fn);
        jest.spyOn(paymentIntegrationService.getState(), 'getCustomer').mockReturnValue(customer);
        jest.spyOn(paymentIntegrationService.getState(), 'getBillingAddress').mockReturnValue(billingAddress);
        ...
        jest.spyOn(braintreeConnectMock.identity, 'lookupCustomerByEmail').mockReturnValue({ customerId: 'customerId' });
    ...
    describe('#initialize()', () => {
        it('throws an error if methodId is not provided', async () => {
            const initializationOptions = {} as CustomerInitializeOptions;
    ...
    describe('#authenticatePayPalConnectUser()', () => {
        it('does not authenticate user if braintree connect is not loaded', async () => {
            ...
            expect(paymentIntegrationService.updateProviderCustomerData).not.toHaveBeenCalled();
        });
        ...
        it('does not authenticate user if email is not defined', async () => {
            ...
            jest.spyOn(paymentIntegrationService.getState(), 'getCustomer').mockReturnValue(customerWithoutEmail);
            jest.spyOn(paymentIntegrationService.getState(), 'getBillingAddress').mockReturnValue(billingWithoutEmail);
        ...
        it('triggers PP Connect authentication flow if customer is detected as PP Connect user', async () => {
            ...
            expect(paymentIntegrationService.updateProviderCustomerData).toHaveBeenCalled();
        });
        ...
        it('successfully authenticate customer with PP Connect', async () => {
            ...
            expect(braintreeConnectMock.identity.triggerAuthenticationFlow).toHaveBeenCalled();
            expect(paymentIntegrationService.updateProviderCustomerData).toHaveBeenCalledWith({
