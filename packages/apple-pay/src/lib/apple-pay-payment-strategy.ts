...
interface ApplePayPromise {
    resolve(value: PaymentIntegrationSelectors | PromiseLike<PaymentIntegrationSelectors>): void;
...
export default class ApplePayPaymentStrategy implements PaymentStrategy {
    ...
    async initialize(options?: PaymentInitializeOptions): Promise<PaymentIntegrationSelectors> {
        if (!options?.methodId) {
    ...
    async execute(payload: OrderRequestBody, options?: PaymentRequestOptions): Promise<PaymentIntegrationSelectors> {
        ...
        const paymentMethod = this._paymentIntegrationSelectors.getPaymentMethodOrThrow(methodId);
