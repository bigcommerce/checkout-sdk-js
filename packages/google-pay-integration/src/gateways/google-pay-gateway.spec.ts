...
describe('GooglePayGateway', () => {
    ...
    describe('#handleCoupons', () => {
        ...
        it('should call removeAllCoupons if there is no coupon codes came in the arguments with "withClickOnTheCartPage" flag true', async () => {
            jest.spyOn(gateway, 'getAppliedCoupons').mockReturnValueOnce({
                offers: [{ redemptionCode: 'code_222', description: 'some description' }],
            });
        ...
        it('should return applied coupon code with "isCartPage" flag true', async () => {
            jest.spyOn(gateway, 'getAppliedCoupons')
                .mockReturnValueOnce({ offers: [] })
                .mockReturnValueOnce({
